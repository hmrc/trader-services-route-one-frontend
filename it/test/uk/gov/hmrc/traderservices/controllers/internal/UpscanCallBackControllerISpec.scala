...
import test.uk.gov.hmrc.traderservices.controllers.CreateCaseJourneyISpecSetup
import test.uk.gov.hmrc.traderservices.stubs.UpscanInitiateStubs
import test.uk.gov.hmrc.traderservices.support.TestData
...
class UpscanCallBackControllerISpec extends CreateCaseJourneyISpecSetup with UpscanInitiateStubs {
  ...
  "POST /callback-from-upscan/new/journey/:journeyId" should {
    "return 400 if callback body invalid" in {
      ...
      val result =
        await(
          request(s"/callback-from-upscan/new/journey/${SHA256.compute(journeyId.value)}/$nonce", true)
    ...
    "change nothing if nonce not matching" in {
      ...
      val result =
        await(
          request(
            s"/callback-from-upscan/new/journey/${SHA256.compute(journeyId.value)}/${Nonce.random}",
            true
          )
            .withHttpHeaders(HeaderNames.CONTENT_TYPE -> MimeTypes.JSON)
            .post(
