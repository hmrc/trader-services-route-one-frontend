...
case class TraderServicesApiRequest(
  ...
  def toUpperCase: TraderServicesApiRequest =
    copy(givenName = this.givenName.toUpperCase, familyName = this.familyName.toUpperCase)

}

object TraderServicesApiRequest {
  implicit val formats = Json.format[TraderServicesApiRequest]
