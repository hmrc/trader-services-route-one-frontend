...
object FormFieldMappings {
  ...
  val epuMapping: Mapping[EPU] = uppercaseNormalizedText
    .verifying(
      first(
        ...
        haveLength("epu", 3),
        constraint[String]("epu", "invalid-number", s => Try(s.toInt).fold(_ => true, epu => epu > 0 && epu <= 669))
      )
    )
  ...
  val explanationTextMapping: Mapping[String] = text
    .verifying(
      first(
        nonEmpty("explanationText"),
        constraint[String]("explanationText", "invalid-length", _.length <= 1000)
      )
    )
