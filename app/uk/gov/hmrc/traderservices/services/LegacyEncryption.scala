...
object LegacyEncryption {

  final def encryptLegacy[T](value: T, keyProvider: KeyProvider)(implicit wrts: Writes[T]): String = {
    val plainJsonString = Json.stringify(wrts.writes(value))
    try {
      ...
      new String(
        Base64.encodeBase64(cipher.doFinal(plainJsonString.getBytes(StandardCharsets.UTF_8))),
  ...
  final def decryptLegacy[T](encrypted: String, keyProvider: KeyProvider)(implicit rds: Reads[T]): T =
    keyProvider.keys
