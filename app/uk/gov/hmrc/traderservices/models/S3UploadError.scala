...
/**
  * Details of the S3 file upload error are supplied as query parameters,
  ...
  *
  * If a redirect URL is not set, the proxy responds with the failure status code.
  * The details of the error along with the key are available from the JSON body
  ...
  * @param errorCode S3 error code as per https://docs.aws.amazon.com/AmazonS3/latest/API/ErrorResponses.html#ErrorCodeList
  */
